...
df = pd.read_csv("data/mineral/mineplant.csv")
...
for state in statelist:
    statedict[state] = len(df.loc[df["STATE_LOCA"] == state])
...
plt.title("Mineral Plants by State")
plt.contourf(colors.values(), 51, cmap="copper")
plt.colorbar(ticks=v)
plt.savefig("output/mineralplant/usmap.png")
plt.close()
